...
button.addEventListener("click", () => {
  if (button.textContent === "Click Me! ❤") {
    ...
    fetch('send_mail.php')
      .then(response => {
        if (response.ok) {
          button.textContent = "te amo mi amor 💗";
        } else {
...
function drawText() {
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["te amo tanto perla, more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("te amo tanto perla, more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
    ...
    if(frameNumber >= 3000 && frameNumber < 99999){
        ...
        context.fillText("quieres pasar conmigo el 14 <3", canvas.width/2, (canvas.height/2 + 120));
